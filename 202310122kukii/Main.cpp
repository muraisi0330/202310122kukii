...
void Main()
{
	...
	const Texture farmEmojiq{ U"🗡️"_emoji };
	...
	while (System::Update())
	{
		...
		const int32 cps = (farmCount + factoryCount*10 + farmCounta * 50);
		...
		// 農場ボタン
		if (Button(Rect{ 340, 40, 420, 100 }, farmEmoji, font, U"鍛冶屋", U"C{} / 1 CPS"_fmt(farmCost), farmCount, (farmCost <= cookies)))
		{
			cookies -= farmCost;
			++farmCount;
		}
		...
		if (Button(Rect{ 340, 280, 420, 100 }, farmEmojiq, font, U"伝説の鍛冶屋", U"C{} / 50 CPS"_fmt(farmCosta), farmCounta, (farmCosta <= cookies)))
		{
			cookies -= farmCosta;
			++farmCounta;
		}
